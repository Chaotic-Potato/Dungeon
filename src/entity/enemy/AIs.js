var AIs = {
	Meele: function(e) {
		...
		e.move(Math.min(Math.abs(dx), Math.max(-1 * Math.abs(dx), e.speed * Math.round(Math.cos(e.dir)))), Math.min(Math.abs(dy), Math.max(-1 * Math.abs(dy), e.speed * Math.round(Math.sin(e.dir)))))
		e.attack(e.dir)
	}
}
